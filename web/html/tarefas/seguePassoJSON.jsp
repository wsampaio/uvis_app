...
ObjTable tarefa = new Tarefa();
tarefa.load(conn);
ObjTable pergunta = new Pergunta();
pergunta.load(conn);
...
for(int i : lista){
	...
	obj.loadPk(i);
	tarefa.loadPk( (int) obj.getValueOf("cod_tarefa"));
	...
	for(Field f : obj.objFields){
		...
		if(f.fieldName.equals("cod_tarefa")){
			objField.put(f.fieldName, new JSONObject(f.toString()).put(
					"tarefa", 
					new JSONObject(tarefa.toString())
			));
		} else {
			objField.put(f.fieldName, new JSONObject(f.toString()));
		};
	}

	String[] params = { (String) obj.getValueOf("cod_passo").toString()};
	int[] perguntas = pergunta.getIndexList("listaPorPasso", params);

	JSONArray perguntasJSON = new JSONArray();

	for(int p : perguntas){
		pergunta.loadPk(p);
		perguntasJSON.put(
			new JSONObject(pergunta.toString())
		);
	}

	objField.put("perguntas", perguntasJSON);
